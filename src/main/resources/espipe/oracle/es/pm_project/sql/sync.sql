...
from
  ...
  pm_project_sd pps,
  (
    select tm_team_object.object_id, listagg(tm_team_member.user_id, ',') WITHIN GROUP (order by null) as user_ids
    from
    tm_team_member, tm_team_object,
      (select tm_team_member.tm_team_id from tm_team_member where
      nvl(tm_team_member.mod_date, tm_team_member.create_date) > ?
      and nvl(tm_team_member.mod_date, tm_team_member.create_date) < ?) team_ids
    where
    tm_team_object.oc_object_class_id = 1000000 and
    tm_team_member.tm_team_id = tm_team_object.tm_team_id and
    tm_team_member.current_record = 1 and
    team_ids.tm_team_id = tm_team_object.tm_team_id
    group by tm_team_object.object_id) pm_member
where pp.pm_project_id = pm_member.object_id
and pp.pm_project_id = pps.pm_project_id
